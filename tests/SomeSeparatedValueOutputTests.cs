...
namespace Tests
{
	public class SomeSeparatedValueOutputTests
	{
		...
		[Test, Description("Make sure CSV handles commas , correctly")]
		public void CsvCommasInInputTest()
		{
			// Arrange
			SomeSeparatedValueOutput csvOutput = new SomeSeparatedValueOutput();
			MemoryStream ms = new MemoryStream();
			string parameter = "csv";
			List<object> headers = new List<object> {"o,ne", "tw,o", "three"};
			List<object> values = new List<object> {1, 2, 3};
			string expectedOutput = $"\"o,ne\",\"tw,o\",three{newLine}1,2,3{newLine}";
